...
## 2026-02-13: UX en Esteganografía (Feedback de Clave Implícita)
- **Desafío**: En sistemas donde la clave es "información a la vista" (ej: la última palabra del texto visible), el usuario puede no ser consciente de qué palabra exacta está utilizando el algoritmo, creando frustración si la desencriptación falla.
- **Mejoras**: Se implementó un indicador visual en tiempo real que muestra explícitamente qué palabra se usará como clave (extrayéndola del texto pegado y limpiando caracteres ocultos).
- **Valor**: Reduce la ansiedad del usuario al confirmar que el sistema "ve" la clave correcta y verifica indirectamente que la normalización de texto está funcionando.
